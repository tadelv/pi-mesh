...
- A selected session that belongs to exactly one confirmed running job also shows a live view. Live frames render into a labelled live tail inside the transcript ("Live — not the durable record"); a dedicated polite region announces entry boundaries only, never token deltas, and the transcript panel itself stays silent. A `not-live`, `error` or `end` frame states the reason, removes the live tail and falls back to the durable page - the missed deltas are never claimed, and a frame for one session never renders over another.
