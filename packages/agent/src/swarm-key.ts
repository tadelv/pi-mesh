...
export function decodeSwarmKey(encoded: string): Uint8Array {
  ...
  if (decoded.length !== SWARM_KEY_BYTES) {
    throw invalidKey(`Swarm key must decode to exactly 32 bytes (got ${decoded.length})`);
  }
...
export async function loadSwarmKey(path?: string): Promise<Uint8Array> {
  ...
  try {
    ...
    throw invalidKey(`Unable to read swarm key at ${keyPath}: ${errorMessage(error)}`, error);
  }

  // Windows reports synthetic modes, so the POSIX gate does not apply there.
  ...
  try {
    ...
    if (error instanceof PiMeshError) {
      throw invalidKey(`Invalid swarm key at ${keyPath}: ${error.message}`, error);
    }
    throw error;
  }
}
