...
describe("swarm key handshake", () => {
  ...
  it("verifies valid, invalid, malformed, and differently keyed HMACs", () => {
    ...
    expect(verifyHandshake(key, expectedHmac.slice(0, -1), encoded)).toBe(
      false,
    );
    expect(verifyHandshake(key, `${expectedHmac}AAAA`, encoded)).toBe(false);
    expect(verifyHandshake(key, "not-base64", encoded)).toBe(false);
    expect(
      verifyHandshake(new Uint8Array(32).fill(0), expectedHmac, encoded),
    ).toBe(false);
  });
  ...
  it("returns false rather than throwing for non-string remote values", () => {
    ...
    expect(verifyHandshake(key, Buffer.from(expectedHmac), encoded)).toBe(
      false,
    );
    expect(verifyHandshake(new Uint8Array(0), expectedHmac, encoded)).toBe(
      false,
    );
  });
});
